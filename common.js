...
//    });
//}
function onImageBabyHeadClicked(ev, babyId) {
    //console.log(ev)
...
function RemoveCircleBabyHead(babyId) {
    let circle = document.getElementById("circle" + babyId + "");
...
function SwitchToFullScreen(goFullScreenWithoutCheckingResolution) {
    ...
    setTimeout(() => {
        if (!$('.room-dropdown-toggler').length) {
            ...
            $('body').click(ev => {
                ...
                if (!isRoomElement)
                    $('.room-dropdown-toggler .room-selected').parent().click();
            });
        }
    }, 500);
}
...
function documentVisibilityChanged(visible) {
...
;
async function RegisterInTaskEvent(taskId, events) {
...
async function ChangeTaskProgress(eventArgs) {
    if (!TaskSignalrConnection) {
        await initializeTasksSignalR();
    }
    try {
...
class ProgressService {
    static async GO(percentage, type) {
        if (globalDotNetObjRef)
            await globalDotNetObjRef.invokeMethodAsync("ProgressService_GO", percentage, type);
    }
}
var UiPageProgressType;
(function (UiPageProgressType) {
    UiPageProgressType[UiPageProgressType["Default"] = 0] = "Default";
    UiPageProgressType[UiPageProgressType["Info"] = 1] = "Info";
    UiPageProgressType[UiPageProgressType["Success"] = 2] = "Success";
    UiPageProgressType[UiPageProgressType["Warning"] = 3] = "Warning";
    UiPageProgressType[UiPageProgressType["Error"] = 4] = "Error";
})(UiPageProgressType || (UiPageProgressType = {}));
