...
declare function moment(x);
declare function syncNotes();

declare var bootstrap: Dictionary<any>


enum StripStatusEnum {
    All = -1,
    SelectAnOption = 0,
    Live = 1,
    Ready = 2,
...
interface IStripDataPoint {
    deceleration?: boolean;
    acceleration?: boolean;

    createdDt: number;
    createdDtAsDate?: Date;
    fhR1: number;
    fhR2: number;
    toco: number;
    note?: IStripNote;
    ...
    original_fhR1?: number;
    original_fhR2?: number;
    original_toco?: number;

    resolved?: boolean;
}

var charts: { [stripId: string]: CTGChart } = {};

var chartResizeTimers: { [stripId: string]: { timer?: number, wait: number } } = {};
...
interface IStripNote {
    id: string
    stripDataCreationTime: string
    stripId: string

    notes: string
    notesType: NotesTypeEnum
    snoozeDuration: string
    sequence: number

}
interface ICTGChartOptions {
    canClassifyAI: any;

    canViewAlertHistory: boolean;
    canEditPatient: boolean;
    canPrint: boolean;
    canViewNotes: boolean;
    canViewStripsAuditLogs: boolean;
    minCreationTime: string;
    divideDuration: number;
    showLatestValuesLabel: boolean;

    /** in minutes */
    range: number;
    data?: Dictionary<IStripDataPoint>;
    data2?: IStripDataPoint[];
    stripId: string;
    laborId: string;
    isAdmitted: boolean;
    patient: IPatientInfo;
    showColumnSizeOptions: any;
...
interface IZipChartOptions {
    canClassifyAI: any;

    stripsChartOptions: { [stripId: string]: ICTGChartOptions },
...
class CTGChart {
    aiClassifiedBands: Highcharts.PlotLineOrBand[] = [];
    prevPoint: IStripDataPoint;
    minTime: number;

    navigateTo(datetime: string | number | Date) {
        ...
        if (typeof datetime == "number")
            datetime = (datetime as number).toDate();
        ...
        this.stockChart.xAxis[0].setExtremes(
            time,
            time + this.options.range * 60 * 1000, true, false
        );
    ...
    showBradycardia_OR_Tachycardia(tachyOrBradyMessage: string) {
    ...
    flashing(tachyOrBradyMessage: string, top: number, height: number) {
    ...
    options: ICTGChartOptions;
    width?: number;
    stockChart?: Highcharts.Chart;
    series_fhr1?: Highcharts.Series;
    series_fhr2?: Highcharts.Series;
    series_toco?: Highcharts.Series;

    series_original_fhr1?: Highcharts.Series;
    series_original_fhr2?: Highcharts.Series;
    series_original_toco?: Highcharts.Series;

    series_fhr1_baseline?: Highcharts.Series;
    series_fhr1_acceleration?: Highcharts.Series;
    series_fhr1_deceleration?: Highcharts.Series;

    note_series?: Highcharts.Series;

    series_sma_fhr1?: Highcharts.Series;

    clearLabelsTimer: number;
    ...
    //get mainParentElement() { return $(`#${this.chartElementId}`).parent('.ctg-chart') }


    fhr1_label?: Highcharts.SVGElement;
    fhr2_label?: Highcharts.SVGElement;
    toco_label?: Highcharts.SVGElement;


    constructor(options: ICTGChartOptions) {
    ...
    private setChartResizeObserver() {
        ...
        let doResize = (entries: ResizeObserverEntry[]) => {

            if (resizeTimer.wait > 0) return;
    ...
    lastBaseline: { time: number, avg: number, avg2: number, fhrPoint: IStripDataPoint, acceleration: boolean, deceleration: boolean } = null;
    celerationCounter = 0;
    isPreviousWas__Celeration = false;
    accelerationPack: number[][] = [];
    decelerationPack: number[][] = [];
    minuteData: { point: IStripDataPoint, baseline: number }[] = []
    addPoints(data: IStripDataPoint[], showDataInLabels = true) {
        ...
        const addPointToSeries = (point: IStripDataPoint, isFake) => {
            ...
            const createBaseline = (windowSize: number, series_fhr1_baseline: Highcharts.Series) => {
    ...
    private resolvePoint(point: IStripDataPoint, prevPoint: IStripDataPoint) {
        if (point.resolved)
            return;
    ...
    private setLabelValues(fhr1_label_value: number, fhr2_label_value: number, toco_label_value: number) {

        //fhr1_label_value += 50
        //fhr2_label_value += 50
        //toco_label_value += 50
        const calcX = (label: Highcharts.SVGElement) => {
            ...
            var w = label.attr("width") as number;
            var fhr1W = this.fhr1_label.attr("width") as number;
            var fhr2W = this.fhr2_label.attr("width") as number;
            var tocoW = this.toco_label.attr("width") as number;
            ...
            return label.attr("translateX") as number;
        }
    ...
    generateChart() {
        ...
        else this.addPoints([{ createdDtAsDate: new Date(), createdDt: new Date().getDate(), fhR1: 0, fhR2: 0, toco: 0, baseline: 0, note: null }], false);
        ...
        if (this.note_series.data == null || this.note_series.data.length <= 0) {
            this.stockChart.series[0].update({ showInNavigator: false } as Highcharts.SeriesOptionsType, true);
        }
    ...
    /** in seconds */
    goToEndDuration = 30;
    private goToEndOfChartTimer: number;
    configureExtremes() {

        if (Math.abs(this.maxX - this.minX) < this.options.range * 60 * 1000) {
            this.stockChart.xAxis[0].setExtremes(
                this.minX,
                this.minX + this.options.range * 60 * 1000, true, false
            );
        }
        else {
            if (this.goToEndOfChart)
                this.stockChart.xAxis[0].setExtremes(
                    this.maxX - this.options.range * 60 * 1000,
                    this.maxX, true, false
                );
        }
    }
    $ctgchartpane = $('<div class="ctg-chart-pane" />');
    menus: Dictionary<{
        text?: string,
        href?(): string,
        onclick?(): void,
    ...
    private createOptions() {
        ...
        var width = $('#' + this.chartElementId).width() as number;
    ...
    findNearestPoint(createdDt: number) {
        var point: IStripDataPoint;
...
async function createChart(stripChartOptions: ICTGChartOptions) {
    ...
    var prevPoint: IStripDataPoint = null;
    var minTime = 0;
    var accelerationPack: number[][] = [];
    var decelerationPack: number[][] = [];
    ...
    data2.forEach((x, i) => {
        ...
        var baseline = null;
        if (i == 0) minTime = timeOfCurrentPoint;
